  ...
  persist(
    (set, get) => ({
      ...
      playSong: (song, queue, startIndex) => {
        ...
        playbackStore.setCurrentSong(song, true);
      ...
      next: () => {
        const { queue, currentIndex, shuffleIndices, currentShuffleIndex } =
          get();
        if (queue.length === 0) return;

        const playbackStore = usePlaybackStore.getState();
        const playMode = playbackStore.playMode;
        ...
        if (nextSong) {
          playbackStore.setCurrentSong(nextSong, true);
          playbackStore.incrementPlayCount();
      ...
      previous: () => {
        ...
        if (playbackStore.currentTime > 3) {
          playbackStore.setCurrentTime(0);
          return;
        }
        ...
        if (prevSong) {
          playbackStore.setCurrentSong(prevSong, true);
          playbackStore.incrementPlayCount();
      ...
      removeFromQueue: (index) => {
        set((state) => {
          ...
          if (newQueue.length === 0) {
            playbackStore.setCurrentSong(null);
          } else if (index === state.currentIndex) {
            playbackStore.setCurrentSong(newQueue[newIndex] || null);
          }
